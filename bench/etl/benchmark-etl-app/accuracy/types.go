...
// JSONファイル全体の構造に合わせて、最上位のキーを定義
type ResultFile struct {
	ConfigGeneral ConfigGeneral `json:"config_general"`
	Results       Results       `json:"results"`
	// config_tasks はキーが動的なため、map[string]interface{} で柔軟に受け取る
	ConfigTasks map[string]interface{} `json:"config_tasks"`
}

// ConfigGeneral は "config_general" オブジェクトに対応
type ConfigGeneral struct {
	ModelConfig                 ModelConfig `json:"model_config"`
	TotalEvaluationTimeSecondes string      `json:"total_evaluation_time_secondes"`
}

// Results は "results" オブジェクトに対応
// キーがタスク名で動的に変わるため、map[string]TaskResultとして定義
type Results map[string]TaskResult

// TaskResult は各タスクの結果を格納
// "acc"や"pass@k"などキーが動的に変わるため、map[string]float64として定義
type TaskResult map[string]float64

// ModelConfig は "model_config" オブジェクトに対応
type ModelConfig struct {
	ModelName            string               `json:"model_name"`
	GenerationParameters GenerationParameters `json:"generation_parameters"`
}

// GenerationParameters は "generation_parameters" オブジェクトに対応
...
// BenchmarkItem はDynamoDBに格納するアイテム
